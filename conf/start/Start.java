...
public class Start
{
   ...
   public static void startMainThread(String var0, String var1, String var2) {
      ...
      if(var0 != null && var1 != null) {
		...
		if(var2 != null) {
			String[] ipAndPort = var2.split(":");
			try {
				Field f;
				f = var7.getClass().getDeclaredField("server");
				f.setAccessible(true);
				f.set(var7, ipAndPort[0]);
				f = var7.getClass().getDeclaredField("port");
				f.setAccessible(true);
				f.set(var7, Integer.parseInt(ipAndPort[1]));
			} catch (Exception ex) {
				ex.printStackTrace();
			}
		}
